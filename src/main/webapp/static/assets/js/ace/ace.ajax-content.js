...
(function($ , undefined) {
	...
	function AceAjax(contentArea, settings) {
		...
		this.getUrl = function(url, hash, manual_trigger) {
			...
			$.ajax({
				...
				beforeSend:function(request){
					request.setRequestHeader("html","true");
				}
			})
					...
					if(link_element.length > 0) {
						...
						if(nav.length > 0) {
							...
							if(active_li.length == 0 ){
								active_li = link_element.closest('li').addClass('active');
							}
							...
							var pid = active_li.attr('data-id');
							if(nav.find('.active').find("[data-id='"+pid+"']").length ==0){
								nav.find('li[data-id]').hide();
								nav.find("li[data-id='"+pid+"']").show();
							}
							$("nav.navbar-menu ul#menu li").removeClass("active");
							$('nav.navbar-menu ul#menu li a[data-id="'+pid+'"]').closest('li').addClass("active");
						}
					}
				}
		...
		/////////////////
		$(window)
		.off('hashchange.ace_ajax reload.ace_ajax')
		.on('hashchange.ace_ajax reload.ace_ajax', function(e, manual_trigger) {
