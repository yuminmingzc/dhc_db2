var addTabs = function (options) {
	...
	if (!$("#" + id)[0]) {
		...
		var tabMenuListHeight = $('#tabs>ul').first().height();
		if (tabMenuListHeight < 35) {
			tabMenuListHeight = 35;
		}
		var padding = 40;  //   校准
		var mainHeight = strs[0] - navbar_h - tabMenuListHeight-padding;
		//创建新TAB的title
	...
	last.contextPopup({
		...
		items: [
			...
			{
				label: '除此之外全部关闭', action: function () {
					var closeText = last.children("a").text().trim();
					var nowText = $("#tabs").find("li[class='active']").children("a").text().trim();
			...
			null,
			{
				label: '当前页右侧全部关闭', action: function () {
					var closeText = last.children("a").text().trim();
					var nowText = $("#tabs").find("li[class='active']").children("a").text().trim();
					if (closeText == nowText) {
						//当前页面
			...
			{
				label: '当前页左侧全部关闭', action: function () {
					var closeText = last.children("a").text().trim();
					var nowText = $("#tabs").find("li[class='active']").children("a").text().trim();
					if (closeText == nowText) {
						//当前页面
