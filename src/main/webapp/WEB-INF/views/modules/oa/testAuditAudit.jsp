	...
	<div class="form-actions">
		<shiro:hasPermission name="oa:testAudit:edit">
			<c:if test="${testAudit.act.taskDefKey eq 'apply_end'}">
				<input id="btnSubmit" class="btn btn-primary btn-sm" type="submit" value="兑 现"
				       onclick="$('#flag').val('yes')"/>&nbsp;
			</c:if>
			<c:if test="${testAudit.act.taskDefKey ne 'apply_end'}">
				<input id="btnSubmit" class="btn btn-primary btn-sm" type="submit" value="同 意"
				       onclick="$('#flag').val('yes')"/>&nbsp;
				<input id="btnSubmit" class="btn btn-inverse btn-sm" type="submit" value="驳 回" onclick="$('#flag').val('no')"/>&nbsp;
			</c:if>
		</shiro:hasPermission>
		<input id="btnCancel" class="btn btn-sm" type="button" value="返 回" onclick="history.go(-1)"/>
	</div>
...
<script type="text/javascript">
	...
	var scripts = [null,'${ctxStatic}/jquery-validation/1.11.1/jquery.validate.min.js', null];
