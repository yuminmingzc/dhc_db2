...
<form:form id="inputForm" modelAttribute="testAudit" action="${ctx}/oa/testAudit/save" method="post"
			...
			<tr>
				...
				<td>
						...
						<%--allowClear="true" notAllowSelectParent="true" smallBtn="false"/>--%>
							<%--此处初始化用户为1--%>
					<c:if test="${empty testAudit.user.id}">
						<input type="hidden" name="user.id" value="1">
					</c:if>
					<c:if test="${not empty testAudit.user.id}">
						<input type="hidden" name="user.id" value="${testAudit.user.id}">
					</c:if>
	...
	<div class="form-actions">
		<shiro:hasPermission name="oa:testAudit:edit">
			<input id="btnSubmit" class="btn btn-primary btn-sm" type="submit" value="提交申请" onclick="$('#flag').val('yes')"/>&nbsp;
