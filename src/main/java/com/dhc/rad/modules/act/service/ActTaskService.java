...
public class ActTaskService extends BaseService {
    ...
    public List<Act> todoList(Act act) {
        ...
        for (Task task : todoList) {
            ...
            ProcessDefinition prodef = ProcessDefCache.get(task.getProcessDefinitionId());
            e.setProcDefVersion(prodef.getVersion());
            e.setProcDefName(prodef.getName());
            e.setProcDefId(task.getProcessDefinitionId());

            e.setProcInsId(task.getProcessInstanceId());

            e.setTaskAssignee(task.getAssignee());
            e.setTaskCreateTime(task.getCreateTime());
            e.setTaskExecutionId(task.getExecutionId());
            e.setTaskId(task.getId());
            e.setTaskDefKey(task.getTaskDefinitionKey());
            e.setTask(task);

            Map<String, Object> var = task.getProcessVariables();
            String title = (String) var.get("title");
            e.setVars(var);
            if (StringUtils.isBlank(title)) {
                e.setVarsMapTitle("");
            } else {
                e.setVarsMapTitle(title);
            }
...
//			e.setProcExecUrl(ActUtils.getProcExeUrl(task.getProcessDefinitionId()));
    ...
    public Page<Act> historicList(Page<Act> page, Act act) {
        ...
        List<Act> pageList = new ArrayList<Act>();
        for (HistoricTaskInstance histTask : histList) {
            Act e = new Act();
            ProcessDefinition prodef = ProcessDefCache.get(histTask.getProcessDefinitionId());
            e.setProcDefVersion(prodef.getVersion());
            e.setProcDefName(prodef.getName());
            ...
            e.setTaskAssignee(histTask.getAssignee());
            e.setTaskEndTime(histTask.getEndTime());
            e.setTaskExecutionId(histTask.getExecutionId());
            ...
            Map<String, Object> var = histTask.getProcessVariables();
            String title = (String) var.get("title");
            e.setVars(var);
            if (StringUtils.isBlank(title)) {
                e.setVarsMapTitle("");
...
//	}
     ...
     * @date 2017/7/27
     */
    public Page<Act> findHistoricSearchPage(Page<Act> actPage, Act act) {
        String userId = UserUtils.getUser().getLoginName();//ObjectUtils.toString(UserUtils.getUser().getId());
        ...
        // 查询总数
        actPage.setCount(histTaskQuery.count());

        // 查询列表
        List<HistoricTaskInstance> histList = histTaskQuery.listPage(actPage.getFirstResult(), actPage.getMaxResults());
        for (HistoricTaskInstance histTask : histList) {
            Act e = new Act();
            e.setHistTask(histTask);
